...
function NewPregnancyForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const [babySex, setBabySex] = useState<string>('n├úo-revelado');
  ...
  const selectedPlan: GestationPlan = planType === 'personalizado'
    ? { type: 'personalizado', totalDays: customDays, label: 'Plano Personalizado', description: `Dura├º├úo: ${customDays} dias` }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const pregRef = await addDoc(collection(db, 'pregnancies'), {
        ...
        doctorName: userData?.name || 'M├®dico Respons├ível',
    ...
    <div className="new-pregnancy-form">
      <div className="npf-steps">
        ...
        <div className={`npf-step ${step >= 3 ? 'active' : ''}`}>
          ...
          <span>Confirma├º├úo</span>
        </div>
      </div>

      {step === 1 && (
        <motion.div className="npf-content" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          <h3 className="npf-section-title">­ƒæ® Cadastro da M├úe do Beb├¬</h3>
          <div className="npf-grid-2">
            <div className="form-group-modern">
              <label className="form-label-modern">Nome completo da m├úe *</label>
          ...
          <div className="access-credentials-box glass-box">
            <p className="ac-title">­ƒöæ Credenciais de Login que o M├®dico Fornece ├á M├úe</p>
            <div className="npf-grid-2">
              <div className="form-group-modern">
                <label className="form-label-modern">E-mail de Acesso da M├úe *</label>
              ...
              <div className="form-group-modern">
                <label className="form-label-modern">Senha Inicial da M├úe</label>
                <input className="form-input-modern" value={motherPassword} onChange={e => setMotherPassword(e.target.value)} placeholder="123456" />
              </div>
            </div>
            <small style={{ color: 'var(--txt-muted)' }}>A m├úe usar├í este e-mail e senha para logar na rota /login.</small>
          </div>

          <h3 className="npf-section-title" style={{ marginTop: 20 }}>­ƒæ¿ Dados do Pai (Opcional)</h3>
          <div className="npf-grid-2">
            <div className="form-group-modern">
          ...
          <h3 className="npf-section-title" style={{ marginTop: 20 }}>­ƒæÂ Dados do Beb├¬</h3>
          <div className="npf-grid-2">
            <div className="form-group-modern">
              <label className="form-label-modern">Nome do beb├¬</label>
              <input className="form-input-modern" value={babyName} onChange={e => setBabyName(e.target.value)} placeholder="Nome da crian├ºa" />
            </div>
            <div className="form-group-modern">
              <label className="form-label-modern">Sexo do Beb├¬</label>
              <select className="form-select-modern" value={babySex} onChange={e => setBabySex(e.target.value)}>
                <option value="n├úo-revelado">N├úo revelado ainda</option>
                <option value="menina">Menina ­ƒæº</option>
                <option value="menino">Menino ­ƒæª</option>
                <option value="g├¬meos-meninas">G├¬meas (Meninas) ­ƒæÂ­ƒæÂ</option>
                <option value="g├¬meos-meninos">G├¬meos (Meninos) ­ƒæÂ­ƒæÂ</option>
                <option value="g├¬meos-misto">G├¬meos (Casal) ­ƒæÂ­ƒæÂ</option>
              </select>
            </div>
          </div>
          ...
          <div className="npf-actions">
            <button className="btn-modern btn-modern-primary" onClick={() => setStep(2)} disabled={!motherName || !motherEmail}>
              Pr├│ximo: Protocolo da Gravidez ÔåÆ
            </button>
          </div>
        </motion.div>
      )}
      ...
      {step === 2 && (
        <motion.div className="npf-content" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          <h3 className="npf-section-title">­ƒôï Protocolo Gestacional</h3>
          <p className="npf-desc">
            Defina a dura├º├úo da gesta├º├úo combinada com a m├úe no jogo. O sistema distribuir├í automaticamente 9 consultas presenciais e exames.
          </p>
          ...
          {planType === 'personalizado' && (
            <motion.div className="custom-days-area" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <label className="form-label">Dura├º├úo total da gesta├º├úo no jogo (em dias reais)</label>
          ...
          <div className="form-group" style={{ marginTop: 20 }}>
            <label className="form-label">Data de in├¡cio da gesta├º├úo *</label>
            <input
              type="date"
              className="form-input"
              value={startDate}
          ...
          <div className="plan-preview glass-box">
            <h4>­ƒôà Resumo do Cronograma Gerado</h4>
            <div className="pp-grid">
              <div><span className="pp-label">In├¡cio</span><strong>{format(new Date(startDate), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}</strong></div>
              <div><span className="pp-label">Dura├º├úo</span><strong>{selectedPlan.totalDays} dias reais</strong></div>
              <div><span className="pp-label">1 M├¬s = </span><strong>{(selectedPlan.totalDays / 9).toFixed(1)} dias reais</strong></div>
          ...
          <div className="form-group" style={{ marginTop: 16 }}>
            <label className="form-label">Notas e Prontu├írio M├®dico</label>
            <textarea
              className="form-input"
              rows={3}
              placeholder="Instru├º├Áes de pr├®-natal, observa├º├Áes dos exames..."
              value={notes}
          ...
          <div className="npf-actions">
            <button className="btn-modern btn-modern-secondary" onClick={() => setStep(1)}>ÔåÉ Voltar</button>
            <button className="btn-modern btn-modern-primary" onClick={() => setStep(3)}>Pr├│ximo: Confirmar ÔåÆ</button>
          </div>
        </motion.div>
      )}
      ...
      {step === 3 && (
        <motion.div className="npf-content" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          <h3 className="npf-section-title">Ô£à Confirmar Prontu├írio</h3>

          <div className="confirm-card glass-box">
            <div className="cc-row"><span>Paciente (M├úe):</span><strong>{motherName} {motherAvatarName ? `(@${motherAvatarName})` : ''}</strong></div>
            <div className="cc-row"><span>E-mail de Login da M├úe:</span><strong>{motherEmail}</strong></div>
            ...
            {babyName && <div className="cc-row"><span>Beb├¬:</span><strong>{babyName} ÔÇö {babySex}</strong></div>}
            <div className="divider" />
            <div className="cc-row"><span>Dura├º├úo Total:</span><strong>{selectedPlan.totalDays} dias reais</strong></div>
            <div className="cc-row"><span>In├¡cio:</span><strong>{format(new Date(startDate), "dd/MM/yyyy")}</strong></div>
            <div className="cc-row"><span>Parto previsto:</span><strong className="gradient-txt">{format(expectedBirth, "dd/MM/yyyy")}</strong></div>
          </div>

          <div className="npf-actions">
            <button className="btn-modern btn-modern-secondary" onClick={() => setStep(2)}>ÔåÉ Voltar</button>
            <button className="btn-modern btn-modern-primary btn-lg" onClick={handleSubmit} disabled={loading}>
              {loading ? 'ÔÅ│ Cadastrando...' : 'Ô£¿ Finalizar Cadastro e Gerar Prontu├írio'}
            </button>
          </div>
        </motion.div>
      )}
    ...
    >
      <div className="pc-header" onClick={() => setExpanded(!expanded)}>
        <div className="pc-avatar">
          {pregnancy.baby?.sex === 'menina' ? '­ƒæº' : pregnancy.baby?.sex === 'menino' ? '­ƒæª' : '­ƒæÂ'}
        </div>
        <div className="pc-info">
          ...
          <p className="pc-sub">
            {pregnancy.baby?.name || 'Beb├¬'} ┬À {pregnancy.gestationPlan.label} ({pregnancy.gestationPlan.totalDays} dias)
          </p>
        </div>
        <div className="pc-right">
          <span className={`badge badge-${pregnancy.currentStatus === 'ativa' ? 'neutral' : 'gold'}`}>
            {pregnancy.currentStatus === 'ativa' ? `M├¬s ${month}` : 'Parto Realizado'}
          </span>
          ...
          <span className="pc-chevron">{expanded ? 'Ôû▓' : 'Ôû╝'}</span>
        </div>
      </div>
        ...
        {expanded && (
          ...
          >
            ...
            <div className="pc-details-grid">
              <div><span className="pp-label">In├¡cio</span><strong>{format(startDate, 'dd/MM/yyyy')}</strong></div>
              <div><span className="pp-label">E-mail da M├úe</span><strong>{(pregnancy as any).motherEmail || 'N├úo cadastrado'}</strong></div>
            ...
            {pregnancy.notes && (
              <div className="pc-notes"><span>­ƒôØ Prontu├írio M├®dico:</span> {pregnancy.notes}</div>
            )}
            <div className="pc-actions">
              {pregnancy.currentStatus === 'ativa' && (
                <button className="btn-modern btn-modern-primary btn-sm" onClick={registerBirth}>
                  ­ƒì╝ Registrar Parto & Emitir Certid├úo
                </button>
              )}
              <button className="btn-modern btn-modern-secondary btn-sm">­ƒôæ Emitir Carteirinha da Crian├ºa</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    ...
    <div className="doctor-panel page-enter">
      <div className="container">
        ...
        <div className="dp-header">
          <div>
            <h1 className="dash-title">
              ­ƒæ¿ÔÇìÔÜò´©Å Painel do Doutor / Administrador
            </h1>
            <p className="dash-subtitle">Gest├úo Hospitalar ÔÇö {userData?.name || 'Dr. M├®dico Chefe'}</p>
          </div>
        ...
        <div className="dp-tabs">
          ...
          >
            ­ƒñ░ Prontu├írio de Pacientes ({pregnancies.length})
          </button>
          <button
            className={`dp-tab ${tab === 'new' ? 'active' : ''}`}
            onClick={() => setTab('new')}
          >
            Ô×ò Cadastrar Nova Paciente (M├úe)
          </button>
          <button
            className={`dp-tab ${tab === 'ultrasound' ? 'active' : ''}`}
            onClick={() => setTab('ultrasound')}
          >
            ­ƒûÑ´©Å Gerador de Ultrassom
          </button>
        </div>
          ...
          <div className="patients-list">
            {loading ? (
              <div className="dp-loading">
                <motion.div style={{ display: 'inline-block', transformOrigin: 'center' }} animate={{ rotate: 360 }} transition={{ duration: 1.5, repeat: Infinity }}>­ƒî©</motion.div>
                <p>Buscando prontu├írios...</p>
              </div>
              ...
              <div className="dp-empty glass-box">
                ...
                <button className="btn-modern btn-modern-primary" onClick={() => setTab('new')}>
                  Ô×ò Cadastrar Primeira Paciente
                </button>
              </div>
            ) : (
              <>
                {active.length > 0 && (
                  <>
                    <h3 className="patients-section-title">Gesta├º├Áes Ativas ({active.length})</h3>
